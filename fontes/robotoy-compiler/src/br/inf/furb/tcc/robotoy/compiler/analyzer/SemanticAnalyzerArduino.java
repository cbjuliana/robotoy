...
public class SemanticAnalyzerArduino implements Constants, GenericSemanticAnalyzer {

    private final GenericCodeGenerator generator;

    private static final String DOUBLE_CONVERSION = "(String) %s";
    ...
    public SemanticAnalyzerArduino(GenericCodeGenerator generator) {
    ...
    private void appendMethodInvoked() {
        String methodName = removerAcentos(token.getLexeme());
    ...
    private void defineMethodName(Token token) {
        ...
        String name = removerAcentos(token.getLexeme());
        method.setName(name);
    ...
    private void appendParameter() {
        String content = parameter.getParameter(false);
        parameter = null;
        appendContent(content);
    }

    private void appendBeep() {
        appendContent("emitirSom");
    }

    private void appendWrite() {
        appendContent("escrever");
    }
    ...
    private void appendHorizontalDirection(Token token) {
        ...
        if ("esquerda".equalsIgnoreCase(direction)) {
            content = content.concat("Esquerda");
        } else if ("direita".equalsIgnoreCase(direction)) {
            content = content.concat("Direita");
        }
    ...
    private void appendVerticalDirection(Token token) {
        ...
        if ("frente".equalsIgnoreCase(direction)) {
            content = content.concat("Frente");
        } else if ("trás".equalsIgnoreCase(direction)) {
            content = content.concat("Tras");
        }
    ...
    private void appendHasObstacle() {
        ...
        conditionPart.appendValue("_temObstaculo()");
    }
    ...
    private void appendOpeningParenthesis() {
        ...
        if (variable != null) {
            if (variable.getType() == Type.TEXT || variable.getType() == Type.NUMBER) {
    ...
    private void appendVariableName(Token token) throws SemanticError {
        String variableName = removerAcentos(token.getLexeme());
    ...
    private void appendVariable(Token token) throws SemanticError {
        String variableNameToAppend = removerAcentos(token.getLexeme());
    ...
    private void appendColorInVariable(RobotoyVariable variable, String colorName) throws SemanticError {
        ...
        Type type = variable.getType();
        if (type == Type.COLOR) {
            String colorDeclaration = "String %s)";
            appendContent(String.format(colorDeclaration, getCorrespondingColor(colorName)));
        } else if (type == Type.TEXT) {
    ...
    private void appendArithmeticOperator(Token token) throws SemanticError {
        ...
        if ("+".equals(arithmeticOperator) || "-".equals(arithmeticOperator) || "*".equals(arithmeticOperator) || "/".equals(arithmeticOperator)) {
            arithmeticOperator = " ".concat(arithmeticOperator).concat(" ");
        } else if ("%".equals(arithmeticOperator)) {
            arithmeticOperator = " ".concat(arithmeticOperator).concat(" (int) ");
        }
